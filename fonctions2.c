...
void display2(char tab[ROW][COL]) { // Affiche le terrain avec les évenements
  int i, j;
  for (i = 0; i < ROW; i++) { // Parcours du plateau
    for (j = 0; j < COL; j++) {
      if (tab[i][j] == 'Z' || tab[i][j] == 'H' || tab[i][j] == 'T' || tab[i][j] == 'B') {
	     switch(tab[i][j]){
		    case 90:	
	        printf("\U0001F9DF ");  //zombie
		      break;
		    case 72:
		      printf("\U0001F9DA "); //Harpie
		      break;
		    case 84:
		      printf("\U0001F479 ");  //Troll
		      break;
		    case 66:
		      printf("\U0001F98E ");  //Basilic
		      break;	
       }
      }
      else if(tab[i][j] == 'o' || tab[i][j] == 'P'){
		    if(tab[i][j] == 'o')
			   printf("\U0001F3FA "); //totem
		    else
			   printf("\U00002B56  "); //portail	
      } 
      else if (tab[i][j] == 'C') {
	 	   printf("\U0001F381 "); //coffre
      }
	    else if(tab[i][j] == 'd' || tab[i][j] == 'e' || tab[i][j] == 'b' || tab[i][j] == 'g'){
		    switch(tab[i][j]){
			   case 98:
			    printf("\U0001F9AF "); //baton
			     break;
			    case 100:
			     printf("\U0001F5E1  "); //dague
			     break;
			    case 101:
			     printf("\U00002694  "); //epee
			     break;
			    case 103:
			     printf("\U0001F4D5 "); //grimoire
			     break;
        } 
      }
...
Hero move(Hero p, char tab[ROW][COL]) {
  ...
  switch (location) { 
    case 1: // On modifie la position du joueur selon la direction qu'il prend
      p.y--;
      break;
    case 2:
      p.x--;
...
Hero back(Hero p,
          char tab[ROW][COL]) { // Lorsque le personnage meurt, s'il tombe sur un totem ou si il est coincé on le renvoie à sa position initiale et on vide son inventaire
  p.x = p.xstart;
  p.y = p.ystart;
  p.stuff[0] = 'v';
  p.stuff[1] = 'v';
